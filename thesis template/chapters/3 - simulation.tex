...
The following graphs show the ballbot stand perfectly still at its
unstable equilibrium point, with initial state $\state = 0$. Both graphs are as expected, as
...
with two things of note. First, is the slight positive $x$ drift in 
...
\ref{fig:uncontrolled_dottheta_both_deviation} start with 
$\state = \bmat{0&0&0.00001&0.0001&0&0&0&0}^\top$. At these initial conditions
we can start to notice some chaotic behavior arising from the interaction
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.74\linewidth]{uncontrolled_dottheta_both_deviation.png}
  ...
  \label{fig:uncontrolled_dottheta_both_deviation}
\end{figure}