...
$q = [x, y, \theta, \phi]$ and then combined to form the full state of the
system.
...
Recalling the definition of a non-linear time-invariant system, they take the
form $\dot{\state} = f(\state, u)$. When calculating $\dot{\state}$ is is
helpful to think of the state as $\dot{\state} = \bmat{\dot{q} \\ \ddot{q}}$.
From this form, one can see that when defining a function that goes from
$\state \rightarrow \dot{\state}$ the $\dot{q}$ states are shared on both sides,
and one only needs to find some function which takes in $q$ and $\dot{q}$
and returns $\ddot{q}$.

The derivation of thus function will be achieved through lagrangian mechanics.
Recall that lagrangian mechanics constructs a function, called the lagrangian
defined as the difference between the kinetic and potential energies of a system
, $\mathcal{L} = T - U$ where $L$ 
